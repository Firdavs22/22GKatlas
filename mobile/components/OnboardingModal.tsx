...
const SLIDES: Record<string, Slide[]> = {
  parent: [
    ...
    { emoji: '⚙️', title: 'Профиль', body: 'Смена пароля, экспорт данных по 152-ФЗ, удаление аккаунта — все через раздел «Профиль».' },
  ],
  teacher: [
    { emoji: '🎓', title: 'Дети группы', body: 'Главная — список ваших детей. Кликните по ребенку для детальной карточки.' },
  ...
  psychologist: [
    { emoji: '👶', title: 'Дети', body: 'Список закрепленных за вами детей с наблюдениями и заметками.' },
    { emoji: '📋', title: 'Рекомендации', body: 'Назначайте задания родителям через мобильное приложение.' },
    { emoji: '📅', title: 'Слоты приема', body: 'Расписывайте окна — родитель забронирует через свое приложение.' },
  ],
  pediatrician: [
    { emoji: '👶', title: 'Дети', body: 'Закрепленные дети с медицинскими пометками (аллергии, особенности).' },
    { emoji: '🩺', title: 'Назначения', body: 'Запись рекомендаций с настройкой видимости — себе / педагогу / родителю.' },
    { emoji: '📅', title: 'Слоты приема', body: 'Окна для записи родителей.' },
  ],
};
