...
export default function PediatricianChild() {
  ...
  const [child, setChild] = useState<{ name?: string; group?: { name?: string }; birthDate?: string } | null>(null);
    ...
    <PageLayout title={child?.name || 'Пациент'}>
      <div className="flex justify-between mb-4">
        <div>
          ...
          {child?.birthDate && <div className="text-sm text-gray-400">Дата рождения: {new Date(child.birthDate).toLocaleDateString('ru')}</div>}
        </div>
