...
const NOTE_TYPES: Record<string, string> = { observation: 'Наблюдение', diagnosis: 'Диагностика', recommendation: 'Рекомендация', plan: 'План работы' };
...
export default function PsychologistChild() {
  ...
  const create = async (e: React.FormEvent) => {
    ...
    setShowForm(false);
    setForm({ type: 'observation', text: '', recommendations: '', visibility: 'specialist_only' });
  };
    ...
    <PageLayout title={child?.name || 'Ребёнок'}>
      ...
      {showForm && (
        <form onSubmit={create} className="bg-white border rounded-xl p-4 mb-4 space-y-3">
          ...
          <textarea value={form.text} onChange={e => setForm(p => ({...p, text: e.target.value}))} placeholder="Заметка..." className="w-full border rounded px-3 py-2 h-24 resize-none" required />
