...
#
# Что делает:
#   1. Создает новую роль с новым именем и сильным паролем
#   2. Передает OWNER базы новой роли
...
docker exec -e PGPASSWORD="$NEW_PASS" "${COMPOSE_PROJECT}-postgres-1" \
  ...
  echo "⚠ Не удалось удалить старого пользователя — возможно, он еще держит соединения. Удалите вручную позже."
